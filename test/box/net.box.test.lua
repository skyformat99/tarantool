...
--
-- gh-946: long call/eval block input.
--

-- Try 2 big requests to occupy both input buffers
-- with one request.
box.schema.user.grant('guest','read,write,execute','universe')
...
longs1 = string.rep('a', 9000)
longs2 = string.rep(longs1, 5)
longs3 = string.rep(longs2, 5)
f1 = fiber.create(function() c:call('long_call', {longs1}) end)
f2 = fiber.create(function() c:call('long_call', {longs2}) end)
c:call('normal_call', {longs3}) -- hangs before 946 fix
c:close()

-- Try multiple little requests to occupy each input buffer with
-- many requests.
